...
export default async function DashboardSupervisorPage() {
  const session = await getSession()
  const accessToken = session?.accessToken ?? ''

  const [stats, bandeja, produccion] = await Promise.all([
    getSupervisorDashboardStats(accessToken),
    getDashboardBandeja(accessToken),
    getDashboardProduccion(accessToken),
  ])
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 flex flex-col gap-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {canVerOrdenes && (
              <Link
                href="/superusuario/carga-trabajo"
