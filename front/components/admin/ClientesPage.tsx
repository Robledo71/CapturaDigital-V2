...
function OCBadge({ requiereOC }: { requiereOC: boolean }) {
  if (requiereOC) {
    return (
      <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-amber-500/10 text-amber-300 border border-amber-500/20">
        Requiere OC
      </span>
    )
  }
  return (
    <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-slate-500/10 text-blue-600 dark:text-slate-400">
      Sin OC
    </span>
  )
}
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-5">
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              <h1 className="text-xl font-bold text-blue-950 dark:text-white">Clientes</h1>
          ...
          {/* Table */}
          <div className="rounded-xl border border-blue-200 dark:border-[#1a2d4d] bg-white dark:bg-[#0c1829] overflow-hidden" aria-label={`Página ${page} de clientes`}>
                    ...
                    filtered.map((cliente) => (
                      <tr key={String(cliente.id)} className="hover:bg-blue-50 dark:hover:bg-[#1a2d4d]/40 transition-colors">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            <div
                              className="w-8 h-8 rounded-lg bg-slate-700 flex items-center justify-center flex-shrink-0 text-blue-700 dark:text-slate-300 text-xs font-bold"
                              aria-hidden="true"
                            >
