    ...
    >
      <div className="bg-white dark:bg-[#0c1829] border border-blue-200 dark:border-[#1a2d4d] rounded-xl shadow-2xl p-6 w-full max-w-sm mx-4">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 id="modal-confirmar-delete-titulo" className="text-blue-950 dark:text-white font-semibold text-sm">
              Eliminar planta
            </h3>
            <p className="text-blue-600 dark:text-slate-400 text-xs mt-0.5">{planta.nombre}</p>
          </div>
        </div>
        <p className="text-blue-700 dark:text-slate-300 text-sm mb-6">
          ¿Eliminar <span className="text-blue-950 dark:text-white font-medium">{planta.nombre}</span>? Esta acción es permanente y no puede revertirse.
        </p>
        <div className="flex gap-3 justify-end">
          ...
          <button
            type="button"
            onClick={onConfirm}
            className="px-4 py-2 text-sm rounded-lg font-medium bg-red-500/80 hover:bg-red-500 text-blue-950 dark:text-white transition-colors"
          >
            Sí, eliminar
          </button>
        </div>
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-5">
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              <h1 className="text-xl font-bold text-blue-950 dark:text-white">Plantas</h1>
          ...
          {/* Table */}
          <div className="rounded-xl border border-blue-200 dark:border-[#1a2d4d] bg-white dark:bg-[#0c1829] overflow-hidden" aria-label={`Página ${page} de plantas`}>
                      ...
                      return (
                        <tr key={String(planta.id)} className="hover:bg-blue-50 dark:hover:bg-[#1a2d4d]/40 transition-colors">
                          <td className="px-4 py-3">
                            <span className="text-slate-200 font-medium">{planta.nombre}</span>
                          </td>
                          <td className="px-4 py-3 text-blue-600 dark:text-slate-400 text-sm">
                            {planta.direccion ?? (
                              <span className="text-slate-600 italic">Sin dirección</span>
                            )}
                          </td>
                          <td className="px-4 py-3">
                            <span className="text-blue-700 dark:text-slate-300 text-sm tabular-nums">
                              {planta.tabletsCount}{' '}
