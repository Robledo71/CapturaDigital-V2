...
describe('getInformalReporteDetalle', () => {
  ...
  it('no expone datos de sesión de captura (fecha_inicio/fin) — quedan en null', async () => {
    vi.mocked(fetch).mockResolvedValueOnce(
      new Response(JSON.stringify({ data: mockApiInformalReport }), { status: 200 }),
    )
    const result = await getInformalReporteDetalle('42', 'tok')
...
describe('registerSamplingDetalleInformal', () => {
  const baseInput = {
    reportId: 10,
    itemId: 1,
    defects: 0,
  ...
  it('respuesta exitosa aprobada → { ok: true, approved: true, ... }', async () => {
    ...
    const result = await registerSamplingDetalleInformal(baseInput)
  ...
  it('respuesta exitosa no aprobada → { ok: true, approved: false, ... }', async () => {
    ...
    const result = await registerSamplingDetalleInformal({ ...baseInput, defects: 2 })
  ...
  it('llama al endpoint correcto con el body correcto', async () => {
    ...
    await registerSamplingDetalleInformal({ ...baseInput, itemId: 7, defects: 2 })
